...
public class RxPermissionsFragment extends Fragment {
    private final static String TAG = RxPermissionsFragment.class.getSimpleName();
    ...
    void onRequestPermissionsResult(String [] permissions, int [] grantResults, boolean [] shouldShowRequestPermissionRationale) {
        for (int i = 0, size = permissions.length; i < size; i++) {
            Log.d(TAG, "onRequestPermissionsResults: " + permissions[i]);
            PublishSubject<Permission> subject = mSubjects.get(permissions[i]);
            if (subject == null) {
                Log.e(TAG, "onRequestPermissionsResults: RxPermissions.onRequestPermissionsResult invoked but didn't find the corresponding permission request.");
                return;
            }
